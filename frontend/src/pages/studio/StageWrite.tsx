...
import { useDemoMeta, useSamplePrompts } from "../../api/demo";
...
export function StageWrite({ flow, identity }: { flow: StudioFlow; identity: DemoIdentity }) {
  ...
  const providers = meta?.providers_available ?? ["fixture"];
  ...
  const onGenerate = async () => {
    ...
    try {
      const res = await generate.mutateAsync({
        ...
        model: flow.model,
        watermark: true,
        watermark_params: { issuer_id: identity.issuerId, model_id: 1, model_version_id: 1, key_id: 1 },
      });
        ...
        <div className={styles.controls}>
            ...
            <div className={styles.pills}>
              {providers.includes("google") && (
                <Pill
                  ...
                  onClick={() => { flow.setProvider("google"); flow.setModel("gemma-3-12b-it"); }}
                >Google · Gemma 3</Pill>
              )}
              <Pill
          ...
          {flow.provider === "google" && (
            <div className={styles.row}>
              ...
              <div className={styles.pills}>
                {["gemma-3-12b-it", "gemma-3-27b-it"].map((m) => (
                  <Pill key={m} active={flow.model === m} onClick={() => flow.setModel(m)}>{m}</Pill>
                ))}
              </div>
            </div>
          )}
