# Veritext v2 — Full Rewrite Specification
...
4. [Core Library: `veritext-watermark`](#4-core-library-veritext-watermark)
5. [Backend: `veritext-server`](#5-backend-veritext-server)
6. [Frontend: `veritext-web`](#6-frontend-veritext-web)
7. [Browser Extension: `veritext-extension`](#7-browser-extension-veritext-extension)
...
Veritext is a provenance-tracking platform for AI-generated text. It answers three questions:
...
| `veritext-registry-demo/` is the "real" app but sounds like a throwaway | Confusing project identity |
...
│
├── src/
│   └── veritext/                     # Main application package
│       ├── __init__.py
│       │
...
| Project root | `veritext-registry-demo/` | `src/veritext/` — proper Python package |
| Auth modules | `registry/auth.py` + `app/auth.py` (name collision) | `veritext/auth/jwt.py` + `veritext/auth/ecdsa.py` |
...
class Identity:
    ...
    issuer_id: int | None = None      # Custom claim: https://veritext.io/issuer_id
...
class FixtureProvider:
    ...
    FIXTURE_TEXT = (
        "Veritext fixture response: this output is deterministic for demo reliability. "
...
### 5.4 Chain Backends (`veritext/chain/`)

```python
# veritext/chain/protocol.py
...
async def create_chain(settings: AppSettings) -> ChainBackend:
    ...
    from veritext.chain.simulated import SimulatedChain
    return SimulatedChain(db_path=settings.db_path)
```

### 5.5 Database Layer (`veritext/db/`)

#### Connection Management

```python
# veritext/db/connection.py
import aiosqlite
...
### 5.6 Services Layer (`veritext/services/`)
...
class ProofBundleBuilder:
    ...
    def bundle_id(payload: dict) -> str:
        """SHA-256 of canonical JSON, prefixed with 'vtb2_'."""
        ...
```

### 5.7 API Routers (`veritext/api/`)

Each router is a focused file handling one domain:

```python
# veritext/api/health.py
...
### 5.8 Application Factory (`veritext/app.py`)
...
async def lifespan(app: FastAPI):
    ...
    logger.info("veritext started", chain_backend=settings.chain_backend, demo_mode=settings.demo_mode, auth0=settings.auth.enabled)
    yield
    # Shutdown
    logger.info("veritext shutting down")

def create_app() -> FastAPI:
    app = FastAPI(title="Veritext", version="2.0.0", lifespan=lifespan)
...
### 5.9 Middleware (`veritext/middleware/`)

```python
# veritext/middleware/logging.py
import structlog
import time
...
## 7. Browser Extension: `veritext-extension`
...
1. **API Resource Server**
   - Identifier: `https://api.veritext.io`
   - Signing: RS256
...
```json
{
  "iss": "https://veritext.us.auth0.com/",
  "sub": "auth0|abc123",
  "aud": "https://api.veritext.io",
  "iat": 1714000000,
  "exp": 1714086400,
  "scope": "openid profile email anchor:create company:create chat:invoke",
  "permissions": ["anchor:create", "company:create", "chat:invoke"],
  "gty": "client-credentials",                           // M2M only
  "https://veritext.io/issuer_id": 42,                  // Custom claim
  "https://veritext.io/email": "agent@company.com"      // Custom claim
}
```
...
```json
{
  ...
  "bundle_id": "vtb2_<sha256_of_canonical_json>",
  "hashing": {
    "algorithm": "sha256",
...
| Field | v1 | v2 |
|---|---|---|
| `spec` | `veritext-proof-bundle/v1` | `veritext-proof-bundle/v2` |
| `bundle_id` prefix | `vtb1_` | `vtb2_` |
...
# === Auth0 (Optional) ===
AUTH0_DOMAIN=                               # e.g., veritext.us.auth0.com
AUTH0_AUDIENCE=https://api.veritext.io
...
VITE_AUTH0_AUDIENCE=https://api.veritext.io
...
2. Create API resource server (`https://api.veritext.io`, RS256, 4 scopes)
...
EXPOSE 5050
CMD ["uv", "run", "uvicorn", "veritext.app:app", "--host", "0.0.0.0", "--port", "5050"]
```
...
| `veritext/auth/ecdsa.py` | sign, verify, recover, keypair generation | None (pure crypto) |
| `veritext/auth/jwt.py` | decode valid token, expired token, bad audience, missing kid, demo fallback | Mock JWKS endpoint |
| `veritext/providers/*` | Each provider's message conversion, error handling | Mock SDK responses |
| `veritext/chain/simulated.py` | anchor, lookup, verify, chain validation, genesis | In-memory SQLite |
| `veritext/chain/solana.py` | anchor success, RPC failure fallback, verify_on_chain | Mock Solana Client |
| `veritext/services/proof_builder.py` | Bundle construction, bundle_id determinism | None |
...
async def test_full_pipeline(client):
    ...
    assert resp.status_code == 200
    assert resp.json()["proof_bundle_v2"]["spec"] == "veritext-proof-bundle/v2"
...
def test_extension_detects_watermark():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_selector(".veritext-detected")
        badge = page.query_selector(".veritext-badge")
...
- [ ] `veritext/config/` — Pydantic Settings with grouped sub-models
- [ ] `veritext/auth/ecdsa.py` — port from `registry/auth.py`
- [ ] `veritext/auth/jwt.py` — port from `app/auth.py`, make JWKS fetch async
- [ ] `veritext/db/` — aiosqlite connection, repositories
- [ ] `veritext/chain/protocol.py` — Protocol + dataclasses
- [ ] `veritext/chain/simulated.py` — async SimulatedChain
- [ ] Unit tests for all above

### Phase 3: Services + Providers (Day 2-3)

- [ ] `veritext/providers/` — base, router, google, minimax, bedrock, fixture
- [ ] `veritext/services/` — ChatService, AnchorService, SigningService, WatermarkService, ProofBundleBuilder
- [ ] `veritext/api/` — all routers with auth dependencies
- [ ] `veritext/app.py` — lifespan factory
- [ ] Integration tests for all API endpoints

### Phase 4: Solana Chain (Day 3)

- [ ] `veritext/chain/solana.py` — async SolanaChain with httpx
- [ ] `veritext/api/solana.py` — verify/balance endpoints
- [ ] `scripts/setup_solana.sh`
...
This rewrite transforms Veritext from a hackathon prototype into a production-grade platform:

- **One monorepo** with clear boundaries (`packages/watermark`, `src/veritext`, `frontend`, `extension`)
